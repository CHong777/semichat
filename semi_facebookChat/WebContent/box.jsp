...
<script type="text/javascript">
	...
	function chatBoxFunction() {
		...
		$.ajax({
			...
			success:function(data){
				...
				//console.log(result[i][0].value.replace(/&nbsp;/gi,''));
					if(result[i][0].value.replace(/&nbsp;/gi,'')==userID) {
	...
	function addBox(lastID, toID, chatContent, chatTime, unread) {
		//console.log(unread);
		var id = toID.replace(/&nbsp;/gi,'');
		$('#boxTable').append('<tr onclick="location.href=\'chat.jsp?toID='+id+'\'">'+
